...
def get_cqt_features(input_file, tags, params=None, output_file=None):
    # type (str, list[str], str) -> PredictedFeatures
    """
    Computes the CQT and returns it as PredictedFeatures.
...
input
    ...
    n_bins = params['cqt_features']['n_bins'] 
    hop_length = params['cqt_features']['hop_length'] 
    target_sr = params['cqt_features']['target_sr']
    bins_per_octave = params['cqt_features']['bins_per_octave']
    ...
    #TODO: Study window parameters

    cqt = np.abs(librosa.core.cqt(y, sr=target_sr, fmin=20.0, sparsity=0, 
        n_bins=n_bins, bins_per_octave=bins_per_octave, hop_length=hop_length))
    
    feats = cqt

    if params['cqt_features']['deltas']:
        d = librosa.feature.delta(feats, order=1)
    else:
        d = np.empty((0,feats.shape[1]))

    if params['cqt_features']['delta_deltas']:
